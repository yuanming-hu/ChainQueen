...
class BatchNormalize:
  def __init__(self, shape, alpha = 0.9):
    ...
    self.mean_ph = tf.placeholder(shape = shape, dtype = tf.float32)
    self.norm_ph = tf.placeholder(shape = shape, dtype = tf.float32)
    self.update_sym = [
...
class SimpleModel:
  def __init__(self, input_channel, output_channel, hidden_layer = 16, batch_normalize = False):
    if batch_normalize:
      ...
      self.bn = None
    self.W1 = tf.Variable(
        0.2 * tf.random_normal(shape=(input_channel, hidden_layer)),
        trainable=True)
    self.b1 = tf.Variable(np.random.random(hidden_layer) * 0.2, trainable=True, dtype = tf.float32)
  
    self.W2 = tf.Variable(
        0.2 * tf.random_normal(shape=(hidden_layer, output_channel)),
        trainable=True)
